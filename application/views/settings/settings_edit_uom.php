...
$unit_of_measurement = array(
  ...
  'value'        => set_value("unit_of_measurement",$uom->unit_of_measurement),
  'class'        => "form-control",
  'autocomplete' => "off",
  'type'         => "text",
...
$description = array(
  'name'         => 'description',
  'id'           => 'description',
  'value'        => set_value("description",$uom->description),
  'class'        => "form-control",
  'autocomplete' => "off",
  'type'         => "text",
     ...
     <div class="row form-group required">
      ...
      <div class="col-md-9">
        <select  name="status" value="" id="status" class="form-control">
          <option value="1" <?php if($uom->status == 1){ echo "selected"; } ?>> <?php echo lang('active');?></option>
          <option value="0" <?php if($uom->status == 0){ echo "selected"; } ?>> <?php echo lang('inactive');?></option>
        </select>
      </div>
    </div>

    <?php echo form_hidden($csrf); ?>
     <?php echo form_hidden('id', $uom->id);?>
  </div>
</div>
