  ...
  <div class="display-table invoice_config">
    <div class="display-margin bordered_tabs">
      <ul class="nav nav-tabs" id="general_tabs">
        ...
        <li class="nav-item"><a class="nav-link" href="#template_poreceiving"><?php echo lang('po_receiving_prefix') ?></a></li>
      </ul>
      <div class="tab-content">
       ...
       <!-- SYSTEM PURCHASE TEMPLATE -->
        <div class="tab-pane form-horizontal" id="template_poreceiving">
          <div class="row-fluid">
              <div class="templates">
                <div class="template">
                  <div class="template-image" style="background-image: url(<?php echo base_url("/assets/img/create_template.jpg") ?>);">
                    <div class="template-actions">
                      <a href="<?php echo site_url("/settings/customize_poreceive_template/") ?>" class="btn btn-secondary btn-block"><?php echo lang("create") ?></a>
                    </div>
                  </div>
                  <h4 class="template-title"><?php echo lang("blank") ?></h4>
                </div>
                <?php foreach (glob("assets/templates/POReceivingTemplate/*", GLOB_ONLYDIR) as $template): ?>
                  <?php
                  $name = pathinfo($template, PATHINFO_BASENAME);
                  $f = file_get_contents(realpath($template)."/template.json");
                  $template_settings = json_decode($f);
                  $isSelected = ($poreceive_settings->name == $template_settings->name)?"selected":"";
                  
                  $template =  str_replace(" ","%20",$template);
                  ?>
                <div class="template <?php echo $isSelected ?>">
                  <div class="template-image" style="width:300px;background-size:300px;background-image: url(<?php echo base_url($template."/preview.jpg") ?>);">
                    <div class="template-actions">
                      <?php if ($poreceive_settings->name == $template_settings->name): ?>
                        <a href="<?php echo site_url("/settings/customize_poreceive_template/".$name) ?>" class="btn btn-secondary btn-block"><?php echo lang("customize") ?></a>
                        <?php else: ?>
                          <a href="<?php echo site_url("/settings/select_poreceive_template/".$name) ?>" class="btn btn-secondary btn-block"><?php echo lang("select") ?></a>
                        <?php endif ?>
                    </div>
                  </div>
                  <h4 class="template-title"><?php echo $template_settings->name ?></h4>
