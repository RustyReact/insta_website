...
function Format_Currency_LeftAlign(number, data, row) {
  if( ROUND_NUMBER != 0 ){
    number = Math.ceil(parseFloat(number) / ROUND_NUMBER) * ROUND_NUMBER;
  }
  number = parseFloat(number).toFixed(DECIMAL_PLACE);
  ...
  if( CURRENCY_PLACE == "left" ){
    if( data == undefined )
      ...
      return "<div class='text-md-left' dir='ltr'><b>"+currency+"</b> "+value+"</div>";
  }else{
    if( data == undefined )
      return value;
    else if( data == true )
      ...
      return "<div class='text-md-left' dir='ltr'>"+value+" <b>"+currency+"</b></div>";
  }
